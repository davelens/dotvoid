...
if ! install_profile_load "$CONFIG"; then
  die "$INSTALL_PROFILE_ERROR"
fi
...
install_profile_fstab "$ROOT_UUID" "$ESP_UUID" > "$TARGET/etc/fstab"
...
# Write the resolved profile rather than copying the config: a config may
# source another one by relative path, which does not resolve in the chroot.
install_profile_dump > "$TARGET/root/install.env"
chmod 600 "$TARGET/root/install.env"
